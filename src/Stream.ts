...
import {IO, chain as chainIO, map as mapIO} from 'fp-ts/lib/IO';
...
export function map<A, B>(ma: Stream<A>, f: (a: A) => B): Stream<B> {
	return async function*() {
		const streamA = await ma();
...
export function chain<A, B>(ma: Stream<A>, f: (a: A) => Stream<B>): Stream<B> {
	return async function*() {
		const streamA = await ma();
		for await (const valA of streamA) {
			const streamB = await f(valA)();
			for await (const valB of streamB) {
				yield valB
			}
		}
	}
...
export class StreamI<A> {
	readonly log: LogEntry<A>[];
	...
	private _put(value: A): void {
		const logEntry: LogEntry<A> = { timestamp: Date.now(), value, offset: this.log.length }
		this._notifySubscribers(logEntry);
		this.log.push(logEntry);
	}
	private _notifySubscribers(entry: LogEntry<A>): void {
		this.subscribers.forEach(subscriber => subscriber(entry));
		this.subscribers = []
	}
	*subscribe(from: number = 0): IterableIterator<Promise<LogEntry<A>>> {
		let logIndex = from;
		while (true) {
			if (isOutOfBound(logIndex, this.log)) {
				yield new Promise(resolve => {
					this.subscribers.push(resolve);
				}) as Promise<LogEntry<A>>;
			} else {
				yield Promise.resolve(this.log[logIndex]);
			}
			logIndex++
		}
	}
